    ...
    <el-card shadow="never" class="card">
      <el-form ref="queryForm" :inline="true" :model="queryParams.model">
        ...
        <el-form-item>
          ...
          <el-button v-hasPermi="['trace:fineSecond:excel']" :loading="btnLoading" type="warning" icon="el-icon-download" size="small" @click="handleExport">导出</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <!-- 表格信息 -->
...
<script>
import { listTraceFineSecond } from '@/api/trace/fineSecond'
import { downloadFineSecond } from '@/api/excel/excel'
...
export default {
  ...
  data() {
    return {
      ...
      column: [
        ...
        { label: 'OP60机床编号', prop: 'cncNo', minWidth: 140 },
        { label: 'OP60开始加工时间', prop: 'startTime', width: 180 },
        { label: 'OP60加工时长', prop: 'processTime', minWidth: 140 },
        { label: 'OP60加工程序号', prop: 'programCode', minWidth: 160 },
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listTraceFineSecond(this.queryParams).then(res => {
    ...
    handleExport() {
      ...
      downloadFineSecond(newData).then((res) => {
