    ...
    <el-card shadow="never" class="card">
      <el-form ref="queryForm" :inline="true" :model="queryParams.model">
        ...
        <el-form-item>
          ...
          <el-button v-hasPermi="['trace:pcd:excel']" :loading="btnLoading" type="warning" icon="el-icon-download" size="small" @click="handleExport">导出</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <!-- 表格信息 -->
...
<script>
import { listTracePcd } from '@/api/trace/pcd'
import { downloadPcd } from '@/api/excel/excel'
...
export default {
  ...
  data() {
    return {
      ...
      column: [
        ...
        { label: 'OP70机床编号', prop: 'cncNo', minWidth: 140 },
        { label: 'OP70开始加工时间', prop: 'startTime', width: 180 },
        { label: 'OP70加工时长', prop: 'processTime', minWidth: 140 },
        { label: 'OP70加工程序号', prop: 'programCode', minWidth: 170 },
        // { label: '中心孔直径检测数据', prop: 'centreHole', width: 180 },
        { label: '产线负责人', prop: 'director', minWidth: 120 },
        { label: '产品型号和规则', prop: 'productCode', minWidth: 170 }
      ],
      loading: false,
      btnLoading: false,
      total: 0, // 列表总条数
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listTracePcd(this.queryParams).then(res => {
