  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="card">
      <el-form ref="queryForm" :inline="true" :model="queryParams">
        <el-form-item prop="productName">
          <el-input v-model="queryParams.productName" clearable size="small" placeholder="产品型号" style="width: 180px;" />
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog v-dialogDrag :close-on-click-modal="false" :title="'新增批次'" :visible.sync="addDialogVisible" class="dialog" width="530px">
      <el-form ref="addFormData" :model="addFormData" :rules="addFormRules" label-width="90px" style=" margin-left:40px;">
        ...
        <el-form-item label="装配顺序" prop="sort">
    ...
    <!--编辑批次追溯-->
    <el-dialog v-dialogDrag :close-on-click-modal="false" :title="'编辑'" :visible.sync="dialogVisible" class="dialog edit-dialog" width="1000px" top="8vh">
      <div class="l-flex l-row-between l-col-center l-m-b-20">
        <div>
          <span class="text-title l-m-l-40">产品名称： <span class="text-info font-bold" style="color: #000">{{ formData.productName || '--' }}</span></span>
        </div>
      ...
      >
        ...
        <el-table-column label="装配顺序" align="center">
          <template slot-scope="scope">
            <el-input
              v-if="isshow[scope.$index]"
            ...
            />
            <!-- 展示状态下显示 -->
            <span v-if="!isshow[scope.$index]">{{ scope.row.sort }}</span>
          </template>
        </el-table-column>
...
<script>
import { addStationProds, deleteWorkStationProds } from '@/api/jingji/componentBatch'
import { listProduct } from '@/api/product/product'
import { addAssembleStep, deleteAssembleStep, listAssembleStep, updateAssembleStep, setNowProd } from '@/api/jingji/assembleSet'
...
export default {
  ...
  data() {
    return {
      ...
      column: [
        ...
        { label: '产品名称', prop: 'productName', disableSort: true },
        // { label: '设置人员', prop: 'updateBy', disableSort: true },
          ...
          return (
            <div>
              ...
              <el-button
                type='primary'
                size='mini'
                onClick={() => { this.handlesetNowProd(scope) }}
              >设为当前产品</el-button>
            </div>
          )
  ...
  methods: {
    ...
    handleDelete(scope) {
      ...
      deleteAssembleStep(scope.row.id).then(res => {
    ...
    handelCheck(index, scope) {
      ...
      updateAssembleStep(scope.row).then(res => {
    ...
    handlesetNowProd(scope) {
      var obj = JSON.parse(JSON.stringify(scope.row))
      setNowProd(obj).then(res => {
        this.$message.success(res.msg)
      })
    },
    ...
    handleEdit(scope) {
      ...
      this.queryParamsDetail.productName = scope.row.productName
      this.queryParamsDetail.productNo = scope.row.productNo
    ...
    getDetailList() {
      ...
      listAssembleStep(this.queryParamsDetail).then(res => {
