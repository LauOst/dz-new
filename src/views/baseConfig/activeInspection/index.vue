...
import { activeList, addActive, delActive, updateActive } from '@/api/inspect/activeInspection'
      ...
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogType !== 'new') { // 编辑
            updateActive(this.formData).then(res => {
            ...
            addActive(this.formData).then(res => {
              this.$message.success(res.msg)
              this.dialogVisible = false
              this.passVisible = false
              this.getList()
