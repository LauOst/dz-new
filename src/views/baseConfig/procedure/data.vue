    ...
    <el-card shadow="never" class="card">
      <el-form ref="queryForm" :inline="true" :model="queryParams">
        <el-form-item prop="equipmentName">
          <el-input v-model="queryParams.equipmentName" v-support size="small" clearable placeholder="工序编号" style="width: 180px;" />
        </el-form-item>
        <el-form-item>
...
import { listWorkpiece, addWorkpiece, listWorkpieceTest, listWorkpieceDetail, updateWorkpiece, delWorkpiece } from '@/api/report/procedure'
...
export default {
  ...
  mounted() {
    this.queryParams.workingProcedureId = this.$route.params.workingProcedureId
    this.getList()
  },
  methods: {
    // 格式化序号
    ...
    changeWork(id) {
      ...
      this.getListTestAll(data[0].productNo)
    },
    // 新增设备
    handleAdd(scope) {
    ...
    handleEdit(scope) {
      ...
      this.handleWorkpiec()
      this.loadingDialog = true
      listWorkpieceDetail(scope.row.workProcedProductId).then(res => {
        this.testList = res.data.dzDetectTempVos
        // this.formData = res.data
        this.formData.productId = String(res.data.product.productId)
        this.formData.workProcedProductId = res.data.workProcedProductId
        this.loadingDialog = false
      })
      ...
      this.$refs['formData'].validate(valid => {
        if (valid) {
          if (this.dialogType === 'edit') {
            ...
            updateWorkpiece(this.formData).then(response => {
            ...
            console.log('新增')
            this.formData.dzDetectTempVos = this.testList
            this.formData.workingProcedureId = this.$route.params.workingProcedureId
            addWorkpiece(this.formData).then(response => {
              this.msgSuccess('修改成功')
              this.dialogVisible = false
              this.getList()
            })
    ...
    handleDelete(scope) {
      ...
      this.$confirm('是否确认删除该数据项?', '警告', {
        ...
        return delWorkpiece(workingProcedureId)
      }).then(() => {
        this.getList()
