    ...
    <el-card shadow="never" class="card">
      <el-form ref="queryForm" :inline="true" :model="queryParams">
        ...
        <el-form-item prop="houseNo">
          <el-input v-model="queryParams.houseNo" clearable size="small" placeholder="库位" style="width: 180px;" />
        </el-form-item>
        <el-form-item prop="htype">
          <el-input v-model="queryParams.htype" clearable size="small" placeholder="类型" style="width: 180px;" />
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          ...
          <el-button v-hasPermi="['productModule:inventoryRecords:excel']" :loading="btnLoading" type="warning" icon="el-icon-download" size="small" @click="handleExport">导出</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <!-- 表格信息 -->
...
<script>
import { listInventoryRecord } from '@/api/productModule/inventoryRecords'
import { downloadEntryAndExitRecord } from '@/api/excel/excel'
...
export default {
  ...
  data() {
    return {
      column: [
        ...
        { label: '库位', prop: 'houseNo', disableSort: true },
        { label: '类型', prop: 'htype', disableSort: true },
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listInventoryRecord(this.queryParams).then(res => {
        this.list = res.data
    ...
    handleExport() {
      ...
      downloadEntryAndExitRecord(newData).then((res) => {
