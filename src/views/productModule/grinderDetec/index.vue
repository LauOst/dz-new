  ...
  <div class="app-container">
    ...
    <el-card shadow="never" class="card">
      <el-form ref="queryForm" :inline="true" :model="queryParams">
        <el-form-item prop="detectionNo">
          <el-select v-model="queryParams.detectionNo" size="small" style="width: 180px;" placeholder="设备">
            <el-option v-for="(item,index) in grinderList" :key="index" :label="item.name" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item>
          ...
          <el-button v-hasPermi="['productModule:grinderDetec:excel']" :loading="btnLoading" type="warning" icon="el-icon-download" size="small" @click="handleExport">导出</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <!-- 表格信息 -->
  ...
  data() {
    return {
      column: [
        ...
        { label: '磨床号', prop: 'detectionNo', disableSort: true, render: (h, scope) => {
          return (
            <span>{scope.row.detectionNo === '2' ? '磨床1' : scope.row.detectionNo === '3' ? '磨床2' : ''}</span>
          )
        } },
        { label: '工件名称', prop: 'productNo', disableSort: true },
        { label: '工件二维码', prop: 'qrCode', disableSort: true },
        { label: '内径', prop: 'detection02', disableSort: true },
        { label: '外径', prop: 'detection01', disableSort: true },
