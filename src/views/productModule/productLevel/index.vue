  ...
  <div class="app-container">
    ...
    <div style="width:100%;height: calc(100vh - 243px);">
      <chart-card shadow height="100%">
        <div v-if="chartData.xaxis.length <= 0" class="l-flex l-col-center l-row-center" style="height: 100%;">
          <div class="l-flex l-flex-col l-col-center">
            <el-image :src="url" fit="cover" />
            <span style="margin-top: 30px;color: #666">数据为空</span>
          </div>
        </div>
        <bar-chart v-else :chart-data="chartData" title="产品等级分布" />
      </chart-card>
    </div>
  </div>
</template>
...
<script>
import { getProductLevel } from '@/api/productModule/productLevel'
  ...
  methods: {
    getList() {
      this.$refs['queryForm'].validate((valid) => {
        if (valid) {
          getProductLevel(this.queryParams).then(res => {
