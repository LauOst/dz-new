  ...
  <div class="app-container">
    <el-card shadow="never" class="card multiple">
      <el-form ref="queryForm" :model="queryParams" :inline="true">
        ...
        <el-form-item v-if="equipmentList !== undefined" prop="equipmentIdList">
          <el-select
            v-model="queryParams.equipmentIdList"
            size="small"
            multiple
            collapse-tags
            placeholder="设备名称"
    ...
    <div style="width:100%;height: calc(100vh - 270px);">
      <chart-card shadow height="100%">
        <div
          v-if="JSON.stringify(lineChartData) === '{}' "
          class="l-flex l-col-center l-row-center"
          style="height: 100%;"
        >
        ...
        <mach-charts v-else :chart-data="lineChartData" />
      </chart-card>
    </div>
...
<script>
import MachCharts from '../components/MachChart'
import { getEquipmentByLineId } from '@/api/data/eqptProduction'
import { machChart } from '@/api/data/machChart'
...
export default {
  ...
  methods: {
    // 获取走势图数据
    getList() {
      machChart(this.queryParams).then(
        response => {
