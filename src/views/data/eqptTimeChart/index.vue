<template>
  <div class="app-container">
    <el-card shadow="never" class="card">
      <el-form ref="queryForm" :model="queryParams" :inline="true">
        ...
        <el-form-item v-if="lineList !== undefined" prop="lineId">
          <el-select v-model="queryParams.lineId" size="small" placeholder="请选择产线" style="width: 100%;" @change="changeLine">
    ...
    <!--            </div>-->
    <!--            <time-chart v-else :chart-data="lineChartData" />-->
    <!--            -->
    <!--          </chart-card>-->
    <!--        </div>-->
    <div style="width:100%;height: calc(100vh - 270px);">
      <chart-card shadow height="100%">
        ...
        <new-time-chart v-else :chart-data="lineChartData" />
...
<script>
import newTimeChart from '../components/newTimeChart'
import { timeChart } from '@/api/data/timeChart'
...
export default {
  ...
  methods: {
    // 获取走势图数据
    getList() {
      timeChart(this.queryParams).then(
        response => {
    ...
    changeLine() {
      [this.queryParams.endTime, this.queryParams.startTime] = this.getMonthDays()
    },
    /** 搜索按钮操作 */
    handleQuery() {
      this.getList()
