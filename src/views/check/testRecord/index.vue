    ...
    <el-card shadow="never" class="card">
      <el-form ref="queryForm" :model="queryParams" :inline="true">
        ...
        <el-form-item>
          ...
          <el-button v-hasPermi="['check:testRecord:excel']" type="warning" icon="el-icon-download" size="small" @click="handleExport">导出</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <dz-table
...
import { listTestRecord } from '@/api/check/testRecord'
...
export default {
  ...
  data() {
    return {
      ...
      uploadUrl: process.env.VUE_APP_BASE_API + '/api/product/check/excel',
      jwt_token: getToken(),
      sub: getSub()
    }
  },
  ...
  methods: {
    ...
    handleExport() {
      ...
      const newData = {}
      this.queryParams.userIdentity = this.userIdentity
      // 循环对象，判断对象中某些属性是否存在，且排除属性值为空格的属性
      for (const key in this.queryParams) {
        if (this.queryParams[key] && this.queryParams[key].toString().replace(/(^\s*)|(\s*$)/g, '') !== '') {
          newData[key] = this.queryParams[key]
        }
      }
      axios.get(
        this.uploadUrl,
        ...
        const fileName = '检测数据-' + this.initDate(new Date(), 3) + '.xlsx'
