<template>
  <div>
    ...
    <el-dialog v-dialogDrag :close-on-click-modal="false" :modal-append-to-body="false" :title="title" :visible.sync="open" width="800px" append-to-body @opened="modalOpened">
      <el-row>
  ...
  methods: {
    ...
    uploadImg() {
      this.$refs.cropper.getCropBlob(data => {
        ...
        // Object.assign(data1, { name1: '333' })
        const formData = new FormData()
        for (var k in data1) {
          console.log('k', k)
          formData.append(k, data1[k])
        }
        // formData.append('picture', data)
        // formData.append('name', '123')
        // formData.append('age', '18')
        // Object.assign(formData, { name1: '333' })
        console.log('formData', formData)
        uploadAvatar(formData).then(response => {
          this.open = false
          // this.options.img = process.env.VUE_APP_BASE_API + '/' + response.data.avatar
          this.options.img = response.data.avatar
